...
// AnÃ¡lisis por reglas/keywords (sin IA).
// Extrae seÃ±ales legales del texto + seÃ±ales tÃ©cnicas (cookies + pageSignals).
...
function parseRetentionDays(textLower) {
  const re = /(\d{1,4})\s*(d[iÃ­]as?|days?|mes(?:es)?|months?|a[nÃ±]os?|years?)/gi;
  let m;
  let best = null;
...
export function analyzeByKeywords(policyText, cookieSummary = {}, pageSignals = {}) {
  ...
  if (!raw.trim()) partialReasons.push("No se pudo leer la polÃ­tica (o no se encontrÃ³ enlace claro).");
  ...
  // SeÃ±ales tÃ©cnicas extra (de pageSignals)
