...
// Llama a Gemini Developer API y devuelve seÃ±ales estructuradas.
// Nota: la IA SOLO extrae seÃ±ales; la letra siempre la calcula scoring.js (determinista).
...
function safeParseJson(txt) {
  ...
  // Intento extraer primer {...} vÃ¡lido
...
export function buildGeminiPrompt({ cookieSummaryText, pageSignalsText, policyText }) {
  return `
Eres un auditor de privacidad. Tu tarea es EXTRAER SEÃ‘ALES, no calcular notas.

Analiza:
1) Resumen tÃ©cnico de cookies:
${cookieSummaryText}

2) SeÃ±ales tÃ©cnicas observadas en la pÃ¡gina (scripts/iframes/dom/storage/cmp):
${pageSignalsText}

3) Texto legal (privacidad/cookies/tÃ©rminos):
${policyText}

Responde SOLO con JSON vÃ¡lido (sin texto extra), con este esquema:
{
...
Reglas:
- NO inventes. Si no estÃ¡ claro: "indeterminado".
