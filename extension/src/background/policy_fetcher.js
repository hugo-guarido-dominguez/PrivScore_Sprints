...
// Descarga la polÃ­tica (HTML/texto) y la convierte a texto limpio.
...
/**
 * Descarga la URL y devuelve el texto limpio para anÃ¡lisis.
...
export async function fetchPolicyText(policyUrl, opts = {}) {
  if (!policyUrl) throw new Error("policyUrl vacÃ­o");
 ...
 * Muy Ãºtil si tienes lista de candidatos (privacy + cookies + terms).
 */
export async function fetchFirstWorkingPolicyText(urlCandidates = [], opts = {}) {
  ...
  throw new Error(`No se pudo descargar ninguna polÃ­tica. Intentos: ${tried.length}`);
}
