...
// IA (si se usa) SOLO extrae seÃ±ales; la letra sale de scoring determinista.
// Flujo: popup -> ANALYZE_TAB
// 1) cookies summary
// 2) inyecta content_script: links + seÃ±ales tÃ©cnicas (terceros/storage/CMP)
// 3) descarga y limpia polÃ­tica (si se encuentra)
// 4) keywords (base) + (opcional) Gemini para mejorar extracciÃ³n legal
...
function mergeAiIntoSignals(ai, baseSignals) {
  ...
  // Si IA indica un nÃºmero, pÃ¡salo (solo si base no lo tenÃ­a)
  const m = ret.match(/(\d{1,4})\s*(d[iÃ­]as?|days?|mes(?:es)?|months?|a[nÃ±]os?|years?)/i);
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  (async () => {
    ...
    // 5) IA opcional (solo extracciÃ³n legal)
    let mode = "keywords";
